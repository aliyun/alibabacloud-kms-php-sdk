...
class AsymmetricEncryptSample
{
    /**
     * 新接入用户可以参考此方法调用KMS实例网关的服务。
     */
    private static function newUserAsymmetricEncryptSample()
    {
        ...
        self::asymmetricEncrypt($client);
    }

    /**
     * 密钥迁移前示例代码
     */
    private static function beforeMigrateAsymmetricEncryptSample()
    {
     ...
     * 密钥迁移后示例代码
     */
    private static function afterMigrateAsymmetricEncryptSample()
    {
        // 创建kms共享网关config并设置相应参数
        $openapiConfig = new Config([
            // 设置访问凭证AccessKeyId
     ...
     * @return void
     */
    private static function asymmetricEncrypt($client)
    {
        try {
            $asymmetricEncryptRequest = new AsymmetricEncryptRequest([
            ...
            $response = $client->asymmetricEncryptWithOptions($asymmetricEncryptRequest, $runtime);
            var_dump($response);
    ...
    {
        self::newUserAsymmetricEncryptSample();
        self::beforeMigrateAsymmetricEncryptSample();
        self::afterMigrateAsymmetricEncryptSample();
    }
}
...
AsymmetricEncryptSample::main(array_slice($argv, 1));