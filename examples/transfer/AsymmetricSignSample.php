...
class AsymmetricSignSample
{
    /**
     * 新接入用户可以参考此方法调用KMS实例网关的服务。
     */
    private static function newUserAsymmetricSignSample()
    {
        ...
        self::asymmetricSign($client);
    }

    /**
     * 密钥迁移前示例代码
     */
    private static function beforeMigrateAsymmetricSignSample()
    {
     ...
     * 密钥迁移后示例代码
     */
    private static function afterMigrateAsymmetricSignSample()
    {
        // 创建kms共享网关config并设置相应参数
        $openapiConfig = new Config([
            // 设置访问凭证AccessKeyId
     ...
     * @return void
     */
    private static function asymmetricSign($client)
    {
        try {
            ...
            $response = $client->asymmetricSignWithOptions($asymmetricSignRequest, $runtime);
            var_dump($response);
    ...
    {
        self::newUserAsymmetricSignSample();
        self::beforeMigrateAsymmetricSignSample();
        self::afterMigrateAsymmetricSignSample();
    }
}
...
AsymmetricSignSample::main(array_slice($argv, 1));