...
class EncryptSample
{
     ...
     */
    private static function newUserEncryptSample()
    {
     ...
     * 密钥迁移前示例代码
     */
    private static function beforeMigrateEncryptSample()
    {
     ...
     * 密钥迁移后示例代码
     */
    private static function afterMigrateEncryptSample()
    {
        // 创建kms共享网关config并设置相应参数
        $openapiConfig = new Config([
            // 设置访问凭证AccessKeyId
     ...
     * @return void
     */
    private static function encrypt($client)
    {
        try {
            ...
            $response = $client->encryptWithOptions($encryptRequest, $runtime);
            var_dump($response);
    ...
    {
        self::newUserEncryptSample();
        self::beforeMigrateEncryptSample();
        self::afterMigrateEncryptSample();
    }
}
...
EncryptSample::main(array_slice($argv, 1));