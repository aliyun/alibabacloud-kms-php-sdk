...
class GenerateDataKeyWithoutPlaintextSample
{
    /**
     * 新接入用户可以参考此方法调用KMS实例网关的服务。
     */
    private static function newUserGenerateDataKeyWithoutPlaintextSample()
    {
        ...
        self::generateDataKeyWithoutPlaintext($client);
    }

    /**
     * 密钥迁移前示例代码
     */
    private static function beforeMigrateGenerateDataKeyWithoutPlaintextSample()
    {
     ...
     * 密钥迁移后示例代码
     */
    private static function afterMigrateGenerateDataKeyWithoutPlaintextSample()
    {
        // 创建kms共享网关config并设置相应参数
        $openapiConfig = new Config([
            // 设置访问凭证AccessKeyId
     ...
     * @return void
     */
    private static function generateDataKeyWithoutPlaintext($client)
    {
        try {
            $generateDataKeyRequest = new GenerateDataKeyWithoutPlaintextRequest([
                "keyId" => "your-key-id"
            ]);
            ...
            $response = $client->generateDataKeyWithoutPlaintextWithOptions($generateDataKeyRequest, $runtime);
            var_dump($response);
    ...
    {
        self::newUserGenerateDataKeyWithoutPlaintextSample();
        self::beforeMigrateGenerateDataKeyWithoutPlaintextSample();
        self::afterMigrateGenerateDataKeyWithoutPlaintextSample();
    }
}
...
GenerateDataKeyWithoutPlaintextSample::main(array_slice($argv, 1));