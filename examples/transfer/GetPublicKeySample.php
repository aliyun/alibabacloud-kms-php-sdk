...
class GetPublicKeySample
{
    /**
     * 新接入用户可以参考此方法调用KMS实例网关的服务。
     */
    private static function newUserGetPublicKeySample()
    {
        ...
        self::getPublicKey($client);
    }

    /**
     * 密钥迁移前示例代码
     */
    private static function beforeMigrateGetPublicKeySample()
    {
     ...
     * 密钥迁移后示例代码
     */
    private static function afterMigrateGetPublicKeySample()
    {
        // 创建kms共享网关config并设置相应参数
        $openapiConfig = new Config([
            // 设置访问凭证AccessKeyId
     ...
     * @return void
     */
    private static function getPublicKey($client)
    {
        try {
            ...
            $response = $client->getPublicKeyWithOptions($getPublicKeyRequest, $runtime);
            var_dump($response);
    ...
    {
        self::newUserGetPublicKeySample();
        self::beforeMigrateGetPublicKeySample();
        self::afterMigrateGetPublicKeySample();
    }
}
...
GetPublicKeySample::main(array_slice($argv, 1));