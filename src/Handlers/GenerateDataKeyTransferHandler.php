...
class GenerateDataKeyTransferHandler extends KmsTransferHandler
{
    /**
     * @param Client $client
    ...
    public function buildDKMSRequest($request, $runtimeOptions)
    {
        ...
        $generateDataKeyDKmsRequest = new GenerateDataKeyRequest([
            "keyId" => isset($query["KeyId"]) ? $query["KeyId"] : null,
            "numberOfBytes" => $numberOfBytes,
        ]);
        if (array_key_exists("EncryptionContext", $query)) {
            $generateDataKeyDKmsRequest->aad = EncryptionContextUtils::encodeEncryptionContext($query["EncryptionContext"]);
        }
        return $generateDataKeyDKmsRequest;
    }

    /**
     * @param GenerateDataKeyRequest $dkmsRequest
     * @param KmsRuntimeOptions $runtimeOptions
